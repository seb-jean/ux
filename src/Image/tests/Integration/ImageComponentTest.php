...
class ImageComponentTest extends KernelTestCase
{
    ...
    /**
     * The <picture> is generated by the component, so it is only reachable through a
     * namespace; the root attributes keep going to the <img>.
     */
    public function testPictureNamespacedAttributesReachTheWrapper(): void
    {
        $html = $this->render('<twig:ux:img src="hero.jpg" alt="Hero" provider="cdn" :widths="[640]" :formats="[\'webp\']" class="rounded" picture:class="block" />');

        self::assertStringContainsString('<picture class="block">', $html);
        self::assertStringContainsString('class="rounded"', $html);
        // the namespaced attribute must not leak onto the <img>
        self::assertStringNotContainsString('picture:class', $html);
    }

    public function testDimensionsFalseDropsTheInferredAttributes(): void
    {
        $html = $this->render('<twig:ux:img src="images/hero.jpg" alt="Hero" :dimensions="false" />');
    ...
    /**
     * Only the inference is switched off: an explicit dimension is still rendered,
     * and no longer drags the other one along through the aspect ratio.
     */
    public function testDimensionsFalseKeepsAnExplicitWidth(): void
    {
        $html = $this->render('<twig:ux:img src="images/hero.jpg" alt="Hero" width="100" :dimensions="false" />');
